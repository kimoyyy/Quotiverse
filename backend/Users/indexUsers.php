...
{
 $sql_query="UPDATE Users SET `status`='".$_GET['status']."' WHERE id=".$_GET['changestatus_id'];
 mysqli_query($conn,$sql_query);
 header("Location: $_SERVER[PHP_SELF]");
}
?>
...
function view_id(id)
{
  window.location.href='view_Users.php?view_id='+id;
}
function delete_id(id)
{
 if(confirm('Sure to Delete ?'))
 {
  window.location.href='indexUsers.php?delete_id='+id;
 }
}
function changestatus_id(id,status)
{
  window.location.href='indexUsers.php?changestatus_id='+id+'&status='+status;
}
</script>
</head>
<body>
...
<div id="container">
 <div id="table-responsive">
    ...
    <tr>
    <th colspan="5"><a href="add_Users.php">add Users.</a></th>
    </tr>
    <th>SL NO</th>
    <th>id</th>
